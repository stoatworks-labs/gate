...
card and reports any that made no difference at all.
    ...
    python3 tools/sweep.py [--binary build/gatest] [--size WxH] [--jobs N]
...
rendered (`gatest --average`), not the last one. Half of this plugin's
controls act in time -- the shutter's flicker, the hold, the weave from frame
to frame, a splice on one frame, the cue dots on four, a hair that comes and
goes -- and the last frame alone can miss them.

**The card moves, and it flashes**, so the hold and the weave have edges to
act on.

**Hair and Splices are rare events**: 150 frames (2.5 s, 60 projector frames),
long enough for the first hair slot's hair and, at 60 splices a minute, for
the seeded print's first splices.

**Cue Dots is an event**: set to 1 before the first frame, it fires on frame 0
and the dots are on the print for projector frames 0-3.

**An option's range is its element count.** `gatest --list` prints it.
...
def main():
    ...
    ap.add_argument("--binary", default=str(ROOT / "build" / "gatest"))
    ...
    scratch = tempfile.mkdtemp(prefix="gasweep")
