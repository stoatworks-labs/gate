/*
 * Stoatworks Labs - About window data for Gate.
 *
 * PROVISIONAL, hand-written in the shape stoatworks-backend/scripts/sync-about.py
 * generates (graticule's recipe). Gate is not yet registered in the website's
 * projects.json, so the sync cannot produce this file; once it is, `sync-about.py
 * --only gate --apply` overwrites this with the generated copy. The facts
 * below are the ones that registration will carry, so the parameter count --
 * which depends on how many links are present -- does not change when it does.
 *
...
{
    inline constexpr auto name = "Gate";
    inline constexpr auto slug = "gate";
    inline constexpr auto hook = "A film projector's gate, shutter and print, for Resolume";
    inline constexpr auto licence = "MIT";
    inline constexpr auto guide = "";
    inline constexpr auto page = "https://stoatworks-labs.com/software/gate/";
    inline constexpr auto repo = "https://github.com/stoatworks-labs/gate";
